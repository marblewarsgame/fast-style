...
class SimpleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img = load_image(self.img_paths[idx], target_shape=(
...
def prepare_img(img_path, target_shape, device, batch_size=1, should_normalize=True, is_255_range=False):
    ...
    if should_normalize:
        transform_list.append(transforms.Normalize(mean=IMAGENET_MEAN_255, std=IMAGENET_STD_NEUTRAL)
                              if is_255_range else transforms.Normalize(mean=IMAGENET_MEAN_1, std=IMAGENET_STD_1))
...
def post_process_image(dump_img):
    assert isinstance(
        dump_img, np.ndarray), f'Expected numpy image got {type(dump_img)}'
...
def get_next_available_name(input_dir):
    ...
    candidates = [candidate for candidate in os.listdir(
        input_dir) if re.fullmatch(img_name_pattern, candidate)]
...
def save_and_maybe_display_image(inference_config, dump_img, should_display=False):
    assert isinstance(
        dump_img, np.ndarray), f'Expected numpy array got {type(dump_img)}.'
    ...
    if inference_config['redirected_output'] is None:
        ...
        dump_img_name = os.path.basename(inference_config['content_input']).split('.')[0] + '_width_' + str(
            inference_config['img_width']) + '_model_' + inference_config['model_name'].split('.')[0] + '.jpg'
        ...
        dump_img_name = get_next_available_name(
            inference_config['redirected_output'])

    # ::-1 because opencv expects BGR (and not RGB) format...
    output_dir = os.path.join(
        dump_dir, dump_img_name)
    cv.imwrite(os.path.join(
        dump_dir, dump_img_name), dump_img[:, :, ::-1])
...
class SequentialSubsetSampler(Sampler):
    ...
    def __init__(self, data_source, subset_size):
        assert isinstance(data_source, Dataset) or isinstance(
...
def get_training_data_loader(training_config, should_normalize=True, is_255_range=False):
    ...
    train_dataset = datasets.ImageFolder(
        training_config['dataset_path'], transform)
    sampler = SequentialSubsetSampler(
        train_dataset, training_config['subset_size'])
    training_config['subset_size'] = len(sampler)  # update in case it was None
    train_loader = DataLoader(
        train_dataset, batch_size=training_config['batch_size'], sampler=sampler, drop_last=True)
    print(
        f'Using {len(train_loader)*training_config["batch_size"]*training_config["num_of_epochs"]} datapoints ({len(train_loader)*training_config["num_of_epochs"]} batches) (MS COCO images) for transformer network training.')
    return train_loader
...
def print_header(training_config):
    print(
        f'Learning the style of {training_config["style_img_name"]} style image.')
    print('*' * 80)
    print(
        f'Hyperparams: content_weight={training_config["content_weight"]}, style_weight={training_config["style_weight"]} and tv_weight={training_config["tv_weight"]}')
    print('*' * 80)

    if training_config["console_log_freq"]:
        print(
            f'Logging to console every {training_config["console_log_freq"]} batches.')
    else:
        print(f'Console logging disabled. Change console_log_freq if you want to use it.')

    if training_config["checkpoint_freq"]:
        print(
            f'Saving checkpoint models every {training_config["checkpoint_freq"]} batches.')
    else:
...
def get_training_metadata(training_config):
    num_of_datapoints = training_config['subset_size'] * \
        training_config['num_of_epochs']
    training_metadata = {
